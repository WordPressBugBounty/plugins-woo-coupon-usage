...
class wcusage_Referrals_Table extends WP_List_Table {
    ...
    function get_bulk_actions() {
        $actions = array(
            ...
            'update_unpaid_commission' => esc_html__( 'Update Unpaid Commission', 'woo-coupon-usage' ),
        );
        return $actions;
    }
    ...
    function column_cb( $item ) {
        ...
        $can_update_commission = false;
        if ( $order && !$paidcommission ) {
            $can_update_commission = wcusage_check_payout_order_status( $order_id, '', $order->get_status() );
        }
        return sprintf( '<input type="checkbox" name="bulk-delete[]" value="%s" data-can-update-commission="%s" />', $item['order_id'], ( $can_update_commission ? '1' : '0' ) );
    }
